import asyncio
from datetime import datetime
from typing import Optional

from fastapi import (APIRouter, Depends, File, Form, HTTPException, Request,
                    UploadFile)
from sqlalchemy import select, update
from sqlalchemy.dialects.mysql import insert as mysql_insert
from sqlalchemy.ext.asyncio import AsyncSession

from auth.jwt_utils import get_current_user
from database.database import get_db
from database.models import (Examiners, GetImages, Images, QuestionBank,
                            StudentWorkbook, UserLog, Users, WorkbookMarking,
                            WorkbookStatus)
from images.s3 import BUCKET_NAME, URL_EXPIRY, get_obj_name, s3,s3_public
from utils.mac_addr_type import MacAddress

router = APIRouter(prefix="/images", tags=["Images"])


@router.post("/upload/question")
async def upload_question_images(
    request: Request,
    workbook_id: str = Form(...),
    question_no: int = Form(...),
    files: list[UploadFile] = File(...),
    checked: bool = Form(...),
    marks: Optional[int] = Form(None),
    mac_addr: MacAddress = Form(...),
    db: AsyncSession = Depends(get_db),
    curr_user: Users = Depends(get_current_user),
):
    if str(curr_user.type) != "admin":
        raise HTTPException(403, detail="Only admins can upload images")
    if checked and marks is None:
        raise HTTPException(400, detail="marks is required when checked is True")
    if all(not file.content_type.startswith("image/") for file in files):  # pyright: ignore[reportOptionalMemberAccess]
        raise HTTPException(status_code=400, detail="Invalid file type")

    workbook_paper = (
        await db.execute(
            select(StudentWorkbook.paper_id).filter(
                StudentWorkbook.workbook_id == workbook_id
            )
        )
    ).scalar_one_or_none()

    if workbook_paper is None:
        raise HTTPException(
            status_code=404,
            detail=f"Workbook '{workbook_id}' was not found. "
                   f"It must be assigned to a student first (Assign Workbook page).",
        )

    question_pages = (
        await db.execute(
            select(QuestionBank.pages).filter(
                QuestionBank.paper_id == workbook_paper,
                QuestionBank.question_no == question_no,
            )
        )
    ).scalar_one_or_none()

    if question_pages is None:
        raise HTTPException(
            status_code=404,
            detail=f"Question {question_no} does not exist for paper '{workbook_paper}'. "
                   f"Create it first (Create Question page).",
        )

    paper_id = (workbook_paper, question_pages)

    if len(files) != paper_id[1]:
        raise HTTPException(
            status_code=400,
            detail=f"Files should be equal to max number of pages. Reqires {paper_id[1]} pages, given {len(files)}, for '{paper_id[0]}', question no '{question_no}'",
        )

    paper_id = str(paper_id[0])
    try:
        for file in files:
            if file.filename is None:
                raise HTTPException(
                    status_code=500, detail="Filename should be the page number"
                )
            try:
                page_no = int(file.filename)
            except Exception as e:
                raise HTTPException(
                    status_code=500, detail=f"Filename is not an int. Error: {e}"
                )

            object_name = get_obj_name(
                workbook_id=workbook_id,
                paper_id=paper_id,
                question_no=question_no,
                page_no=page_no,
                checked=checked,
            )
            file_data = await file.read()

            await asyncio.to_thread(
                s3.put_object,
                Bucket=BUCKET_NAME,
                Key=object_name,
                Body=file_data,
                ContentType=file.content_type,
            )

            existing_image = (
                await db.execute(
                    select(Images).filter_by(
                        workbook_id=workbook_id,
                        question_no=question_no,
                        page_no=page_no,
                        checked=checked,
                    )
                )
            ).scalar_one_or_none()

            if existing_image:
                # Re-upload for this exact page — replace the object_key
                # instead of crashing on the composite primary key.
                existing_image.object_key = object_name
            else:
                image_record = Images(
                    workbook_id=workbook_id,
                    question_no=question_no,
                    page_no=page_no,
                    checked=checked,
                    object_key=object_name,
                )
                db.add(image_record)
            if checked:
                await db.execute(
                    update(WorkbookMarking)
                    .where(
                        WorkbookMarking.workbook_id == workbook_id,
                        WorkbookMarking.question_no == question_no,
                    )
                    .values(marks=marks, submit_time=datetime.now())
                )
                await db.execute(
                    update(WorkbookStatus)
                    .where(
                        WorkbookStatus.workbook_id == workbook_id,
                        WorkbookStatus.question_no == question_no,
                    )
                    .values(checked=True)
                )

        ip_addr = request.client.host if request.client is not None else ""
        user_log = UserLog(
            user_id=curr_user.id,
            mac_addr=mac_addr,
            ip_addr=ip_addr,
            action="upload_image",
            time=datetime.now(),
        )
        db.add(user_log)

        await db.commit()
        return {"message": "Upload successful"}

    except Exception as e:
        await db.rollback()
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/upload/checked")
async def upload_checked_images(
    request: Request,
    workbook_id: str = Form(...),
    question_no: int = Form(...),
    files: list[UploadFile] = File(...),
    page_numbers: str = Form(...),  # comma-separated, e.g. "1,2,3" — same order as files
    mac_addr: MacAddress = Form(...),
    db: AsyncSession = Depends(get_db),
    curr_user: Users = Depends(get_current_user),
):
    """
    Examiner-only. Uploads the flattened (image + annotations baked in)
    version of each answer page, saved as separate checked=True rows so the
    original unmarked scan (checked=False) is never overwritten. This is
    what a student should later see as their "checked" result — pen marks,
    highlights, ticks/crosses all visible, exactly as the examiner drew them.
    """
    if str(curr_user.type) != "examiner":
        raise HTTPException(403, detail="Only examiners can upload checked images")

    try:
        page_nos = [int(p.strip()) for p in page_numbers.split(",") if p.strip() != ""]
    except ValueError:
        raise HTTPException(400, detail="page_numbers must be a comma-separated list of integers")

    if len(page_nos) != len(files):
        raise HTTPException(
            400,
            detail=f"page_numbers count ({len(page_nos)}) must match files count ({len(files)})",
        )

    # Verify the examiner is actually assigned to this paper+question
    paper_id = (
        await db.execute(
            select(StudentWorkbook.paper_id).filter_by(workbook_id=workbook_id)
        )
    ).scalar_one_or_none()
    if paper_id is None:
        raise HTTPException(404, detail=f"Workbook '{workbook_id}' not found")

    is_assigned = (
        await db.execute(
            select(Examiners).filter_by(
                examiner_id=curr_user.id,
                paper_id=paper_id,
                question_no=question_no,
            )
        )
    ).scalar_one_or_none()
    if is_assigned is None:
        raise HTTPException(
            403, detail=f"You are not assigned to question {question_no} of paper '{paper_id}'"
        )

    try:
        for file, page_no in zip(files, page_nos):
            content_type = file.content_type or ""
            # A checked submission is now a single flattened PDF containing every
            # page of the question (built client-side), so accept application/pdf
            # in addition to plain images (kept for backwards compatibility with
            # any old per-page image uploads).
            if not (content_type.startswith("image/") or content_type == "application/pdf"):
                raise HTTPException(
                    400,
                    detail=f"File for page {page_no} is not an image or PDF (got '{content_type}')",
                )

            object_name = get_obj_name(
                workbook_id=workbook_id,
                paper_id=str(paper_id),
                question_no=question_no,
                page_no=page_no,
                checked=True,
            )
            file_data = await file.read()

            await asyncio.to_thread(
                s3.put_object,
                Bucket=BUCKET_NAME,
                Key=object_name,
                Body=file_data,
                ContentType=file.content_type,
            )

            # Replace any previous checked image for this exact page (e.g. a
            # re-submission) rather than accumulating duplicates.
            existing = (
                await db.execute(
                    select(Images).filter_by(
                        workbook_id=workbook_id,
                        question_no=question_no,
                        page_no=page_no,
                        checked=True,
                    )
                )
            ).scalar_one_or_none()
            if existing:
                existing.object_key = object_name
            else:
                db.add(
                    Images(
                        workbook_id=workbook_id,
                        question_no=question_no,
                        page_no=page_no,
                        checked=True,
                        object_key=object_name,
                    )
                )

        ip_addr = request.client.host if request.client is not None else ""
        user_log = UserLog(
            user_id=curr_user.id,
            mac_addr=mac_addr,
            ip_addr=ip_addr,
            action="upload_checked_images",
            time=datetime.now(),
        )
        db.add(user_log)
        await db.commit()
        return {"message": "Checked images uploaded successfully", "pages": page_nos}

    except HTTPException:
        await db.rollback()
        raise
    except Exception as e:
        await db.rollback()
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/upload/page")
async def upload_image(
    request: Request,
    workbook_id: str = Form(...),
    question_no: int = Form(...),
    page_no: int = Form(...),
    file: UploadFile = File(...),
    mac_addr: MacAddress = Form(...),
    db: AsyncSession = Depends(get_db),
    curr_user: Users = Depends(get_current_user),
):
    if str(curr_user.type) != "admin":
        raise HTTPException(403, detail="Only admins can upload images")
    if not file.content_type.startswith("image/"):  # pyright: ignore[reportOptionalMemberAccess]
        raise HTTPException(status_code=400, detail="Invalid file type")

    paper_id = (
        await db.execute(
            select(StudentWorkbook).filter(StudentWorkbook.workbook_id == workbook_id)
        )
    ).first()

    if paper_id is None:
        raise HTTPException(
            status_code=500, detail="Unable to find paper_id for workbook"
        )
    paper_id = str(paper_id.paper_id)
    object_name = get_obj_name(
        workbook_id=workbook_id,
        paper_id=paper_id,
        question_no=question_no,
        page_no=page_no,
        checked=False,
    )
    file_data = await file.read()

    try:
        await asyncio.to_thread(
            s3.put_object,
            Bucket=BUCKET_NAME,
            Key=object_name,
            Body=file_data,
            ContentType=file.content_type,
        )
        image_record = Images(
            workbook_id=workbook_id,
            question_no=question_no,
            checked=False,
            page_no=page_no,
            object_key=object_name,
        )
        db.add(image_record)
        ip_addr = request.client.host if request.client is not None else ""
        user_log = UserLog(
            user_id=curr_user.id,
            mac_addr=mac_addr,
            ip_addr=ip_addr,
            action="upload_image",
            time=datetime.now(),
        )
        db.add(user_log)
        await db.commit()
        return {"message": "Upload successful"}
    except Exception as e:
        await db.rollback()
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/get")
async def get_images(
    images: GetImages,
    request: Request,
    db: AsyncSession = Depends(get_db),
    curr_user: Users = Depends(get_current_user),
):
    if str(curr_user.type) != "examiner":
        raise HTTPException(403, detail="Only examiners can fetch workbook images")
    urls = {}
    try:
        image_rows = (
            await db.execute(
                select(Images).filter(
                    Images.workbook_id == images.workbook_id,
                    Images.question_no == images.question_no,
                    Images.checked == False,
                )
            )
        ).all()

        # Unpack each row to get every uploaded page (not just the first).
        for (image_key,) in image_rows:
            file_url = s3_public.generate_presigned_url(
                ClientMethod="get_object",
                Params={"Bucket": BUCKET_NAME, "Key": image_key.object_key},
                ExpiresIn=URL_EXPIRY,  # seconds
            )
            urls[image_key.page_no] = file_url

        if urls:  
            upsert_stmt = mysql_insert(WorkbookMarking).values(
                workbook_id=images.workbook_id,
                question_no=images.question_no,
                open_time=datetime.now(),
            )
            upsert_stmt = upsert_stmt.on_duplicate_key_update(
                open_time=upsert_stmt.inserted.open_time
            )
            await db.execute(upsert_stmt)

        ip_addr = request.client.host if request.client is not None else ""
        user_log = UserLog(
            user_id=curr_user.id,
            mac_addr=images.mac_addr,
            ip_addr=ip_addr,
            action="get_images",
            time=datetime.now(),
        )
        db.add(user_log)
        await db.commit()
    except Exception as e:
        await db.rollback()
        raise HTTPException(500, detail=f"While in get images: {e}")

    max_marks = None
    paper_id = (
        await db.execute(
            select(StudentWorkbook.paper_id).filter_by(
                workbook_id=images.workbook_id
            )
        )
    ).scalar_one_or_none()
    if paper_id is not None:
        max_marks = (
            await db.execute(
                select(QuestionBank.max_marks).filter_by(
                    paper_id=paper_id, question_no=images.question_no
                )
            )
        ).scalar_one_or_none()

    return {"urls": urls, "max_marks": max_marks}